...
GET_CANDIDATES="$PROG output/prisoners_plain.ttl output/unit_candidates.ttl http://candidates/unit $ARPA_URL/warsa_actor_units --prop http://ldf.fi/schema/warsa/prisoners/unit -n -c -r 3 -w 3"
...
DISAMBIGUATE="$PROG disambiguate_validate sparql/units.sparql output/unit_all.ttl output/unit_linked_validated.ttl http://ldf.fi/schema/warsa/prisoners/warsa_unit $WARSA_ENDPOINT_URL/sparql -n --prop http://candidates/unit -r 3 -w 3"
